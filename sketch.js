...
if(appState===15){
   background(bg2);
    ...
    newBack=createButton('BACK');
    newBack.position(displayWidth/2-780,displayHeight/2-485);
    newBack.size(170,50)
    newBack.style('background','yellow');
...
if(appState===20){
    ...
    stroke(255);
    strokeWeight(2);
    text(phoenixhr,displayWidth/2+150,displayHeight/2-300);
}
if(appState===21){
    background(bg2);
    indiaBut.hide();
...
if(appState===22){
    ...
    stroke(255);
    strokeWeight(2);
    text(pittsburghhr,displayWidth/2+150,displayHeight/2-300);
}
if(appState===23){
    background(bg2);
    indiaBut.hide();
...
async function getpittsburghime(){
    var reponsepittsburgh=await fetch("https://api.weatherapi.com/v1/current.json?key=469320127ae8429099f105012212407&q&q=pittsburgh&aqi=no");
    var responseJSONpittsburgh=await reponsepittsburgh.json();
    pittsburghhr=await responseJSONpittsburgh.location.localtime.slice(11,16);
}
async function getphoenixTime(){
    var responsephoenix=await fetch("https://api.weatherapi.com/v1/current.json?key=469320127ae8429099f105012212407&q&q=phoenix/usa&aqi=no");
    var responseJSONphoenix=await responsephoenix.json();
    phoenixhr=await responseJSONphoenix.location.localtime.slice(11,16);
}